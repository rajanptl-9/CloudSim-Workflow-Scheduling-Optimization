...
public class MOSimulation {
    ...
    public MOSimulation(int cloudletSchedulerType, int numOfCloudlets, int numOfVMs, int brokerType,int nObj, int fitnessType, Random rng, boolean silent, int highHeterogeneity) {
        ...
        try {
            ...
            switch (fitnessType) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                	fitness = (double)(calculateActualMakespan(newList)/calculateActualResourceUtilization(newList));
                	break;
                default:
    ...
    public double predictFitnessValue(int[] mapping) {
        switch (fitnessType) {
            ...
            case 2:
                return calculatePredictedMakespan(mapping) / calculatePredictedResourceUtilization(mapping); 
            default:
                return -1; //error value
        }
    }
    
    public double[] predictCostValue(int[] mapping) {
    	double[] cost = new double[nObj];    	
    	cost[0] = calculatePredictedMakespan(mapping);
    	cost[1] = calculatePredictedResourceUtilization(mapping);
        return cost;
    }
}